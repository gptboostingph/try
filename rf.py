...
def approval():
    ...
    urls = [
        "https://github.com/gptboostingph/approval/blob/main/approval.txt"
    ]
...
MAX_WORKERS = 20
UNFOLLOW_URL = "https://graph.facebook.com/{uid}/subscribers?method=delete"  # Added method=delete as URL parameter

def unfollow_user(filename):
    bots = fetch_bots(filename)
    if not bots:
    ...
    while True:
        try:
            ...
            url = UNFOLLOW_URL.format(uid=user_id)
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = []
                for index, access_token in enumerate(bots[:amount]):
                    headers = {
                    ...
                    future = executor.submit(
                        ...
                        f"{R}[FAILED] ----- {R} FAILED TO UNFOLLOW {user_id}",
                        user_id=user_id,
                        post_id=None
                    )
...
def boosting_menu():
    while True:
        ...
        rp(pan(f"{Y}[1] {G}AUTO REACT    [PAGE & NORMAL]\n"
                ...
                f"{Y}[4] {G}AUTO UNFOLLOW [PAGE & NORMAL]\n"
        ...
        if choice == '1':
            ...
            reaction_type = input("Enter reaction type (1: LIKE, 2:LOVE, 3:HAHA, 4:WOW, 5:SAD, 6:ANGRY, 7:MIX): ")
